...
Validates financial invariants, guardrails, and chaos mitigations.
"""
...
def test_credit_scoring_nominal_budget():
    """Validates that credit application unit cost remains below the 0.0005 EUR NFR threshold."""
    llm = BankingLLMMock(default_model="gpt-4o-mini")
    workload = CreditScoringWorkload(llm)

    # 1st call warms up prefix cache
    workload.process_application("APP_0001", 3000.0, 150000.0, 500.0, 720)

    # 2nd call reuses prefix cache
    resp = workload.process_application("APP_0002", 3200.0, 160000.0, 550.0, 740)

    assert resp.cost_eur < 0.0005, f"Unit cost ({resp.cost_eur:.6f} EUR) exceeds the NFR ceiling."
    assert resp.cached_tokens > 0, "Prefix prompt caching should have been activated."

def test_cache_busting_anomaly_mitigation(chaos_injector):
    """Validates that prompt cache busting causes a quantifiable surge detected by QA tests."""
    ...
    assert res.savings_percent > 30.0, "Cache protection must deliver at least 30% cost savings."

def test_silent_fallback_circuit_breaker(chaos_injector):
    """Validates that FallbackCircuitGuard trips and caps expensive public cloud failover."""
...
def test_denial_of_wallet_guard_blocks_heavy_docs(chaos_injector):
    """Validates that Denial of Wallet attacks are blocked at 100% with zero token expenditure."""
    res = chaos_injector.run_denial_of_wallet_scenario(n_heavy_docs=10)
    assert res.anomaly_detected is True
    assert res.protected_cost_eur == 0.0, "Oversized documents must be rejected at zero EUR cost."
    assert res.savings_percent == 100.0

def test_wealth_copilot_sliding_window_inflation():
    """Validates that a sliding context window prevents quadratic token cost inflation on long chats."""
    ...
    assert cost_prot < cost_unprot, "Sliding context window must reduce cost on extended multi-turn sessions."
